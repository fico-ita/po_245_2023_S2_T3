"""
Carrega os dados fundamentalistas
"""
import os
import warnings
...
def Func_ler_dados_acoes(Ticker, data_inicial, data_simulacao):
    """Reads and filters data for a specific stock.

    Parameters:
        Ticker (str): The ticker symbol of the stock.
        data_inicial (str): The initial date for filtering the data in the 'YYYY-MM-DD' format.
        data_simulacao (str): The simulation date for filtering the data in the 'YYYY-MM-DD' format.

    Returns:
        df_acao (pd.DataFrame): DataFrame containing normalized data for the specified stock.
        df_multiplos (pd.DataFrame): DataFrame containing daily multiples data for the specified stock.
        df_CAGR (pd.DataFrame): DataFrame containing Compound Annual Growth Rate (CAGR) data for the specified stock.

    The function reads and filters data for a specific stock based on the provided ticker symbol, initial date, and simulation date. It returns three DataFrames: one for normalized stock data, one for daily multiples data, and one for CAGR data.

    Example of usage:
        df_acao, df_multiplos, df_CAGR = Func_ler_dados_acoes('AAPL', '2022-01-01', '2023-01-01')
    """
...
def Func_ativos_elegiveis(data_referencia):
    """Determines eligible assets based on a reference date and monthly IBOVESPA data.

    Parameters:
        data_referencia (str): The reference date in the 'YYYY-MM-DD' format.

    Returns:
        lista_ativos_elegiveis (list): List of eligible assets as of the reference date.
        todos_ativos_IBOV (list): List of all assets in IBOVESPA.

    The function determines eligible assets based on a reference date and the monthly data of IBOVESPA. It returns two lists: one containing eligible assets as of the reference date and another containing all assets in IBOVESPA.

    Example of usage:
        lista_ativos_elegiveis, todos_ativos_IBOV = Func_ativos_elegiveis('2023-01-01')
    """
    # Define o caminho do arquivo .parquet que contém os dados mensais do IBOVESPA
    endereco_IBOV = os.path.join(
        "..", "..", "dataset", "BR", "ACOES", "ibov_comp.parquet"
    )
...
def load_data(data_simulacao, dados_portfolio=False):
    """Load financial data for simulation and portfolio construction.

    Parameters:
        data_simulacao (str): The simulation date in the 'YYYY-MM-DD' format.
        dados_portfolio (bool, optional): If True, load data for portfolio creation. If False, load data for benchmark. Default is False.

    Returns:
        dict_df_acoes (dict): Dictionary containing stock data for eligible assets.
        df_benchmark (pd.DataFrame): DataFrame containing benchmark data.
        df_Expectativa_Selic (pd.DataFrame): DataFrame containing SELIC expectations data.

    The function loads financial data for simulation and portfolio construction. It includes stock data for eligible assets, benchmark data, and SELIC expectations data. The optional parameter 'dados_portfolio' allows choosing whether to load data for portfolio creation or benchmark.

    Example of usage:
        dict_df_acoes, df_benchmark, df_Expectativa_Selic = load_data('2023-01-01', dados_portfolio=True)
    """
