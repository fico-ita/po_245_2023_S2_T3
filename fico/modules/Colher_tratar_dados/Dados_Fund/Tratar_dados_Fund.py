...
def endereco_arquivos_brutos(Ticker):
    """Gera os caminhos dos arquivos brutos relacionados ao ticker de uma ação.

    A função recebe o ticker de uma ação e gera os caminhos completos para os
    arquivos brutos relacionados aos dados fundamentais, cotações diárias,
    dados de proventos, dados de eventos (como desdobramentos e grupamentos)
    e dados de subscrições.

    Args:
        Ticker (str): Ticker da ação para a qual os caminhos dos arquivos brutos serão gerados.

    Returns:
        tuple: Uma tupla contendo os caminhos completos para os arquivos brutos de dados
        fundamentais, cotações diárias, dados de proventos, dados de eventos e dados de subscrições,
        respectivamente.
...
def endereco_arquivos_normalizados(Ticker):
    """Gera os caminhos dos arquivos normalizados relacionados ao ticker de uma ação.

    A função recebe o ticker de uma ação e gera os caminhos completos para os
    arquivos normalizados relacionados aos dados fundamentalistas e cotações diárias.

    Args:
        Ticker (str): Ticker da ação para a qual os caminhos dos arquivos normalizados serão gerados.

    Returns:
        tuple: Uma tupla contendo os caminhos completos para os arquivos normalizados de dados
        fundamentalistas e cotações diárias, respectivamente.
...
def Func_Classe_acao(Ticker):
    """Define a classe da ação com base no código do ticker.

    A função recebe o ticker de uma ação e determina a classe da ação com base
    nos caracteres específicos do ticker.

    Args:
        Ticker (str): Ticker da ação para o qual a classe será definida.

    Returns:
        str: A classe da ação, que pode ser "ON" (Ordinária), "PN" (Preferencial),
        "PNA" (Preferencial Classe A), "PNB" (Preferencial Classe B), "UNT" (Unit),
        ou "ERRO" se o ticker não corresponder a nenhuma classe conhecida.
...
## Juntar as listas
Lista_ativos_busca_Total = Lista_ativos_busca + Lista_ativos_busca2 + Ativos_adicionais
