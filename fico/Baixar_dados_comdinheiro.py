""" Baixar os dados fundamentalistas e cotações do site comdinheiro
"""
...
def Baixar_dados(username, password, Fund, Cot, Eventos, lista_ativos):
    """Downloads financial data for stocks.

    This function downloads financial data for stocks, including fundamental data,
    historical stock prices, and events such as dividends, splits, and subscriptions.

    Args:
        username (str): Username for API authentication.
        password (str): Password for API authentication.
        Fund (bool): Flag to download fundamental data if True.
        Cot (bool): Flag to download historical stock prices if True.
        Eventos (bool): Flag to download events data if True.
        lista_ativos (list): List of stock tickers to download data for.

    Returns:
        None

    Example:
        >>> Baixar_dados("your_username", "your_password", True, True, True, ["AAPL", "GOOGL"])
    """
        ...
        for ticker in Lista_ativos_busca_Total:
            ...
            try:
                ...
                if Nao_existe_subscricao is False:
                    ...
                    ## Proventos
                    nome_arquivo = f"../../dataset/BR/ACOES/Dados_Brutos/{ticker}_Subscricao.parquet"
                    # Data como index
